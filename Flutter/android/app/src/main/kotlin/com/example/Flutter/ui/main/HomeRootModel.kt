...
import io.driverdoc.testapp.data.model.notifi.NotifiRequest
...
class HomeRootModel(appDatabase: AppDatabase, interactCommon: InteractCommon, schedulers: Executor) :
        BaseViewModel<HomeRootCallBack>(appDatabase, interactCommon, schedulers) {
    ...
    fun getTrip(auth: String): Disposable? {
    ...
    fun notifi(token: String, platform: String, id_user: String, auth: String) {
        val request = NotifiRequest()
        request.token = token
        request.platform = platform
        request.idDriver = id_user
    ...
    fun getTripDone(auth: String): Disposable? {
        val headerPare = Pair(io.driverdoc.testapp.common.Constants.AUTHORIZATION, Constants.BEARER + auth)
        val header = mutableListOf<Pair<String, String>>()
        header.add(headerPare)
        restApi = ApiHelp.createRetrofit(endpoint = io.driverdoc.testapp.common.Constants.BASE_URL, formatDate = io.driverdoc.testapp.common.Constants.LIST_FORMAT_TIME, headers = header)
